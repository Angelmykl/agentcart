...
export default function BudgetApproval({
  ...
  async function handleApprove() {
    ...
    try {
      ...
      // Step 2: Ask Freighter to sign it
      setStatus("signing");
      const freighter = await import("@stellar/freighter-api");

      let signedXDR: string;

      // Handle both old and new Freighter API versions
      if (typeof (freighter as any).signTransaction === "function") {
        ...
        // result can be string or { signedTxXdr: string }
      ...
      // Step 3: Submit signed transaction to Stellar
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 20, marginBottom: 6 }}>
            Approve Agent Budget
          </div>
          <div style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.6 }}>
            Your Freighter wallet will pop up once to approve the budget. The agent pays for tools automatically from there.
          </div>
        </div>
        ...
        <div style={{
          ...
          marginBottom: 16, fontSize: 12, color: "var(--muted)", lineHeight: 1.6,
        }}>
          <div style={{ fontSize: 10, letterSpacing: 1.5, color: "var(--muted)", marginBottom: 6 }}>TASK</div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10, fontSize: 12 }}>
            ...
            <span style={{ color: "var(--text)", fontFamily: "monospace" }}>
              {userPublicKey.slice(0, 8)}…{userPublicKey.slice(-6)}
            </span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10, fontSize: 12 }}>
          ...
          <div style={{
            ...
            <span style={{ color: "var(--stellar)", fontWeight: 700, fontSize: 18 }}>
              {budgetXlm} XLM
            </span>
          </div>
          <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 8, textAlign: "right" }}>
            Unused XLM is refunded after the task completes
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{
              flex: 2,
              background: ["building", "signing", "submitting"].includes(status)
                ? "var(--border)" : "var(--stellar)",
              color: ["building", "signing", "submitting"].includes(status)
                ? "var(--muted)" : "#000",
              border: "none", borderRadius: 7,
              padding: "12px", fontSize: 13,
              fontWeight: 700, fontFamily: "'Space Mono', monospace",
              cursor: ["building", "signing", "submitting"].includes(status) ? "default" : "pointer",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
              transition: "all 0.2s",
            }}
          >
            {["building", "signing", "submitting"].includes(status) && (
              <span style={{ animation: "pulse 1s infinite" }}>◉</span>
            )}
            {statusLabel[status]}
          </button>
