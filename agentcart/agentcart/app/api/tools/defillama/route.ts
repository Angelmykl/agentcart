...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const top = protocols
      ...
      .slice(0, limit)
      .map((p: { name: string; tvl: number; category: string; chains: string[]; change_1d: number; change_7d: number }) => ({
        name: p.name,
        tvl: p.tvl,
    ...
    return NextResponse.json({
      ...
      _summary: `Retrieved top ${top.length} DeFi protocols. #1: ${top[0]?.name} with ${top[0]?.tvlFormatted} TVL.`,
    });
  } catch {
    return NextResponse.json({
      ...
      retrievedAt: new Date().toISOString(),
      x402: { paid: true, txHash: receipt.txHash },
      _summary: `Top ${limit} DeFi protocols by TVL (mock data).`,
      _mock: true,
    });
  }
}
